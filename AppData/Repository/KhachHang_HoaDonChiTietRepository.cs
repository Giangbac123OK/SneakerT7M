...
{
    public class HoaDonChiTietRepos : KhachHang_IHoaDonChiTietRepository
    {
        ...
        public async Task<List<HoadonchitietViewModel>> HoadonchitietTheoMaHD(int id)
        {
            return await _context.hoadonchitiets.Where(x => x.Idhd == id)
                .Select(x => new HoadonchitietViewModel
                {
                    ...
                    Tensp = _context.sanphams.FirstOrDefault(e => e.Id == _context.Sanphamchitiets.FirstOrDefault(e => e.Id == x.Idspct).Idsp).Tensp,
                    urlHinhanh = _context.sanphams.FirstOrDefault(e => e.Id == _context.Sanphamchitiets.FirstOrDefault(e => e.Id == x.Idspct).Idsp).UrlHinhanh,
                    Giasp = x.Giasp,
                    Giamgia = x.Giamgia ?? 0,
                    Soluong = x.Soluong,
                ...
                if (soluongdatra > 0)
                {
                    if (soluongdatra < hdct.Soluong)
                    {
                        var data = new HoadonchitietViewModel
                        {
                            ...
                            Tensp = _context.sanphams.FirstOrDefault(e => e.Id == _context.Sanphamchitiets.FirstOrDefault(e => e.Id == hdct.Idspct).Idsp).Tensp,
                            urlHinhanh = _context.sanphams.FirstOrDefault(e => e.Id == _context.Sanphamchitiets.FirstOrDefault(e => e.Id == hdct.Idspct).Idsp).UrlHinhanh,
                            Giasp = hdct.Giasp,
